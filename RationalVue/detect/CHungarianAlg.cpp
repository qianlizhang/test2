...
void AssignmentProblemSolver::FeatureMatching(CObjectRect &var)
//void AssignmentProblemSolver::FeatureMatching(vector<vector<double>>& DistMatrix, vector<int>& Assignment)
{
	count = 0;
	//确定是否需要进行匹配的条件，只要追踪轨迹中个数不为空时就要进行匹配（无论检测物体的个数是空还是非空）
	if (var.Cost.size() == 0)
	{
		return;
	}
	vector<vector<double>> DistMatrix = var.Cost;
	vector<int> Assignment = var.assignment;
	TMethod Method = AssignmentProblemSolver::optimal;
	//将二维cost矩阵转化为一维
	...
	var.not_assigned_tracks = var.not_assigned_tracks;
	/*DistMatrix.clear();
	Assignment.clear();*/
	//信号机制传递函数
	//Matching_end_Sig();
...
void AssignmentProblemSolver::assignmentoptimal(int *assignment, double *cost, double *distMatrixIn, int nOfRows, int nOfColumns)
{
	...
	bool *coveredColumns;//记录一列中有没有零值
	bool *coveredRows;//记录一行中有没有零值
	bool *starMatrix;
	bool *newStarMatrix;
	bool *primeMatrix;
	...
	*cost = 0;
	...
	if (nOfRows <= nOfColumns)
	{
		...
		/* Steps 1 and 2a */
		//找出代价矩阵中为零的值，若其所处的行和列中没有带*的0，则对该位置上的0加*
		for (row = 0; row < nOfRows; row++)
		{
...
void AssignmentProblemSolver::step3(int *assignment, double *distMatrix, bool *starMatrix, bool *newStarMatrix, bool *primeMatrix, bool *coveredColumns, bool *coveredRows, int nOfRows, int nOfColumns, int minDim)
{
	...
	/* move to step 5 */
	if (nOfRows*nOfColumns == 1)
	{
		distMatrix = new double[1];
		distMatrix[0] = 1;
	}
